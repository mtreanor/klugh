...
// to (a) associate a rule's leading comment lines with it for display, and
// (b) locate the exact line span of a rule for in-place edit/delete without
// touching any other rule's formatting.
//
// A rule block spans its leading comment lines (contiguous `#` lines immediately
// above the header, with no blank gap) through the last body line (the last
// non-blank line before the next rule's block). Blank lines between rules are
// separators owned by no rule.

const HEADER_RE = /^\s*rule\s+"((?:[^"\\]|\\.)*)"/;
const COMMENT_RE = /^\s*#/;
const BLANK_RE = /^\s*$/;

// Parse raw file text into an ordered array of rule blocks. Each block:
...
export function parseRuleBlocks(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (HEADER_RE.test(lines[i])) headerLines.push(i);
  }

  const blocks = [];
  for (let h = 0; h < headerLines.length; h++) {
    const headerLine = headerLines[h];
    const name = lines[headerLine].match(HEADER_RE)[1];
    ...
    // next rule's block begins (its comment start, or its header if uncommented).
    const nextHeader = headerLines[h + 1];
    let scanEnd = nextHeader === undefined ? lines.length : nextHeader;
    // Exclude the next rule's leading comment lines from this block.
    ...
    // rule and must not be absorbed here — only comments directly above a rule
    // are associated with it (see commentStart above).
...
// Render a rule block from parts. `body` is the DSL under the header (conditions
// and effects); `comment` (optional) becomes `#` lines above the header.
...
// Append a new rule block to a file, separated from existing content by a blank line.
export function appendRule(path, { name, comment, body }) {
  const existing = readFileSync(path, 'utf-8');
  const block = renderRuleBlock({ name, comment, body });
...
// Replace the rule named `targetName` in-place with a re-rendered block. Only the
// matched block's lines change; everything else is byte-preserved.
export function replaceRule(path, targetName, { name, comment, body }) {
  const text = readFileSync(path, 'utf-8');
  const lines = text.split('\n');
  const block = findBlock(text, targetName);
  const rendered = renderRuleBlock({ name, comment, body });
...
// Delete the rule named `targetName`, including its leading comments and one
...
function findBlock(text, targetName) {
  const blocks = parseRuleBlocks(text);
  const matches = blocks.filter(b => b.name === targetName);
  if (matches.length === 0) throw new Error(`No rule named "${targetName}" found in file`);
  if (matches.length > 1) throw new Error(`Multiple rules named "${targetName}" in file — cannot edit unambiguously`);
  return matches[0];
}
