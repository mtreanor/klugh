...
export class TemporalChainPredicate extends Predicate {
  ...
  // where the previous step was asserted at previousTick (-1 = no previous step).
  chainSatisfied(steps, i, previousTick, handler, evaluationContext) {
    ...
    for (const tick of ticks) {
      ...
      if (this.chainSatisfied(steps, i + 1, tick, handler, evaluationContext)) return true;
    }
    return false;
  }
